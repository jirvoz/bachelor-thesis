...
multi-core CPUs came a requirement for multiple runqueues for each core and
their balancing. This brought some complexity to the code and it took some time
to eliminate bugs and tune the scheduler behavior. Another change came with
...
Scheduling on those systems is still under an active development and therefore
it is prone to performance degradation bugs. In order to discover these bugs and
to keep performance of the operating system, performance testing is essential.
Due to this fact, it is essential to evaluate performance of the scheduler
before each release.
...
domains\;\cite{understanging-linux-kernel}. Scheduling domain is a set
...
% TODO
%\subsection{numatop and numastat}

\subsection{perf}
Linux command \texttt{perf}, also known as \texttt{perf\_events}, is tool for
...
It offers wide range of commands, from which are most used:
\paragraph{\texttt{perf stat}} counts selected events during an execution of
process or specified time period. It can observe events belonging to the process
or system-wide. The counted statistics are written at the end of time interval
or process execution.
\paragraph{\texttt{perf record}} record the events to perf.data file for later
analysis.
\paragraph{\texttt{perf report}} read the perf.data file created by perf record
and displays the collected statistics.
\paragraph{\texttt{perf top}} provides live analysis of system by showing all
observed function calls ordered by number of cycles spent in them.
...
Box plots are great to illustrate dispersion and variation of data that do not
